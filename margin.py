...
price = client.get_margin_price_index(symbol=symbol)['price']
...
def margin_create_limit_sell():
    index_price_with_sell_profit_and_precision = round(
        float(price) * float(secrets.choice(sell_profit) + fees), get_price_precision(symbol))
...
def margin_create_limit_buy():
    index_price_with_buy_profit_and_precision = round(
        float(price) * float(secrets.choice(buy_profit) - fees), get_price_precision(symbol))
