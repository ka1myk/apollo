...
def get_tradeable_pair_on_margin():
    temp = []
    ...
    return margin_tradeable_pairs


@tg_alert
def go_baby_margin():
    for x in get_tradeable_pair_on_margin():

        symbol = x
        symbol_info = client.get_symbol_info(x)
        ...
        def margin_create_market_buy():
            client.create_margin_order(symbol=symbol,
                                       quoteOrderQty=round_step_size(float(get_min_notional()) * set_greed(),
                                                                     get_tick_size()),
                                       side='BUY',
                                       type='MARKET'
                                       )
